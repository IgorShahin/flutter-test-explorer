...
class FlutterOutlineRefreshTest : BasePlatformTestCase() {
    fun testRefreshAfterPsiCacheClearStillRecognizesUnchangedSource() {
        val file = myFixture.configureByText("sample_test.dart", "void main() { test('real', () {}); }")
        ...
        try {
            ...
            assertEquals("Refresh must reuse the official outline when only PSI caches changed", 1, index.testCalls(file).size)
    ...
    fun testDuplicateOutlineAfterPsiInvalidationDoesNotInvalidateDiscoveryAgain() {
        val file = myFixture.configureByText("sample_test.dart", "void main() {}")
    ...
    fun testRepeatedFullRefreshAfterPsiInvalidationKeepsRunnableFileInCache() {
        ...
        try {
            index.recordOutline(file, AnalyzerOutline(0, file.textLength, null, listOf(
                AnalyzerOutline(file.text.indexOf("test("), 1, "UNIT_TEST_TEST", emptyList()))))
            val discovery = DartTestDiscovery(project, object : TestRunnabilityValidator {
                private var calls = emptyMap<Int, DartTestKind>()
                override fun prepare(files: List<DartFile>) { calls = index.testCalls(files.single()) }
                override fun kind(call: DartCallExpression) = calls[call.textOffset]
