...
| Extra arguments | Flutter `TestFields.additionalArgs`; Dart `DartTestRunnerParameters.testRunnerOptions`. Runners parse these using IntelliJ argument utilities. `arguments`/VM options are not substitutes for test-runner options. |
| Arbitrary subset | Flutter `TestFields.useRegexp(true)` and Dart `MULTIPLE_NAMES` support regex names. They do not supply a general unique runtime identity for custom wrappers, duplicates, or dynamic registration. Explorer uses a documented fail-closed partial-file/group policy, not an invented regex target. |
| Run All exclusions | Explicit included files, sequential official Run sessions. A partially hidden file blocks the entire batch before starting. Hidden same-name collisions also block named runs. |
...
Outline updates have no document-version field. The adapter binds cached data to a SHA-256 digest of the source contents and applies the official length check; stale/out-of-order analyzer notifications remain an upstream protocol limitation. Do **not** bind outlines to `PsiFile.modificationStamp`: IDEA 253's `PsiFileImpl.clearCaches()` increments that counter without editing source, and the analyzer will not publish a replacement outline merely because PSI caches were cleared. Run targets also carry an in-memory modification stamp and are rejected if the source PSI changed after discovery. This is not a guarantee that arbitrary Dart code compiles or the test passes—only that the IDE recognizes and can construct a test target.
