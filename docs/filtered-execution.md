...
Exact full names are trusted only where the analyzer's reported spelling is provably the runtime name, for the node and for every enclosing group. The spelling is source text, so interpolation, non-raw escapes, adjacent-literal concatenation and triple-quoted strings are conservatively unknown; such a node stays in the tree and stays part of its file's run, but carries no name target, and partial selection in that file fails closed. Wrappers must preserve the standard group/description naming contract; arbitrary dynamic registration or name rewriting is not supported. No runtime discovery or Dart-code evaluation is added.
