...
Discovery dynamically finds package `test/` and `integration_test/` directories under the opened project, including nested packages and arbitrary subdirectories. Both roots appear when they contain runnable tests; missing or empty roots/branches are omitted. Every Dart file under those roots is a candidate: neither the filename nor a `main()` entrypoint is required or treated as proof of runnability. The tree itself comes from the **same analyzer UNIT_TEST_TEST / UNIT_TEST_GROUP entities used by Flutter gutter markers and by Dart-Code in VS Code**, including custom APIs the analyzer recognizes through `@isTest` / `@isTestGroup`. Unopened files receive outline subscriptions too. There is no method-name fallback, no syntactic nesting rule and no project-specific path/name list.
